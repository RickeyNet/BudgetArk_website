...
TOOLS = [
    {
        ...
        'how': [
            'Everything compounds monthly, so the monthly rate <em>r</em> is the annual return divided by 12 and <em>n</em> is the number of months.',
            'A lump sum <em>L</em> left alone grows to <code>L × (1 + r)<sup>n</sup></code>. Monthly contributions <em>P</em>, added at the end of each month, grow to <code>P × ((1 + r)<sup>n</sup> − 1) / r</code>. "Both together" is simply the two added up.',
            'Growth is the ending balance minus what you put in - the lump sum, the contributions, or both. The crossover note finds the first year the monthly plan\'s balance passes the lump sum\'s. The Rule of 72 divides 72 by the annual return to estimate the doubling time.',
        ],
        'example': 'At 7% over 20 years, $40,000 invested once grows to about $161,550, while $500 a month grows to $260,463 - though the monthly plan puts in $120,000 against the lump sum\'s $40,000. The monthly plan overtakes the lump sum in year 10. Doing both ends at about $422,013.',
        'faqs': [
            ('Lump sum or monthly contributions - which is better?', 'Per dollar invested, the lump sum wins: it compounds from day one, so $40,000 more than quadruples in 20 years at 7%. But a monthly plan keeps adding new money, so over a long enough horizon its total pulls ahead - here in year 10. The real answer is to do both: invest what you have now, then keep contributing.'),
